...
const InputField: React.FC<InputFieldProps> = ({
  ...
  const variantClasses = {
    default: clsx(
      ...
      "focus:border-emerald-500 focus:ring-2 focus:ring-teal-200",
      "transition-all duration-200",
      errorMessage && "border-red-500 focus:border-red-500 focus:ring-red-200"
    ),
    priceline: clsx(
      "border-2 border-gray-200 bg-white rounded-xl",
      "focus:border-emerald-600 focus:ring-4 focus:ring-emerald-100",
  ...
  // ✅ Safely determine value/checked based on input type
  const inputProps =
    type === "checkbox"
    ...
    >
      <div className="relative w-full flex items-center gap-2">
        <input
          ...
          disabled={disabled}
          maxLength={maxLength}
          {...inputProps} // ✅ cleanly applied, fully typed
          className={clsx(
            type === "checkbox"
              ? "w-5 h-5 accent-emerald-600 cursor-pointer"
              : clsx(
                  "w-full outline-none text-gray-900 placeholder-gray-500 bg-transparent",
                  "px-4 font-medium",
                  sizeClasses[size],
